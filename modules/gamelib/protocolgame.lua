...
function ProtocolGame:onExtendedOpcode(opcode, buffer)
    ...
    if callback then
        ...
        if status ~= 'E' and status ~= 'P' then extendedJSONData[opcode] = '' end
...
function ProtocolGame.registerOpcode(opcode, callback)
    if opcodeCallbacks[opcode] then error('opcode ' .. opcode .. ' already registered will be overriden') end
...
function ProtocolGame.registerExtendedOpcode(opcode, callback)
    if not callback or type(callback) ~= 'function' then error('Invalid callback.') end
...
function ProtocolGame.registerExtendedJSONOpcode(opcode, callback)
    ...
    if extendedJSONCallbacks[opcode] then error('Opcode is already taken.') end
...
function ProtocolGame.unregisterExtendedJSONOpcode(opcode)
    ...
    if not extendedJSONCallbacks[opcode] then error('Opcode is not registered.') end
...
function ProtocolGame:sendExtendedJSONOpcode(opcode, data)
    if opcode < 0 or opcode > 255 then error('Invalid opcode. Range: 0-255') end
    if type(data) ~= 'table' then error('Invalid data type, should be table') end

    local buffer = json.encode(data)
    local s = {}
    for i = 1, #buffer, maxPacketSize do s[#s + 1] = buffer:sub(i, i + maxPacketSize - 1) end
    if #s == 1 then
        self:sendExtendedOpcode(opcode, s[1])
        return
    end
    self:sendExtendedOpcode(opcode, 'S' .. s[1])
    for i = 2, #s - 1 do self:sendExtendedOpcode(opcode, 'P' .. s[i]) end
