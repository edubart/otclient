...
function onSelectReason(reasonLabel, focused)
    if reasonLabel.actionFlags and focused then
        ...
        for actionBaseFlag = 0, #rvactions do
            ...
            if bit32.band(reasonLabel.actionFlags, math.pow(2, actionBaseFlag)) >
                0 then
...
function report()
    ...
    local statement = ruleViolationWindow:getChildById('statementText')
                          :getText()
    local statementId = 0 -- TODO: message unique id ?
    local ipBanishment = ruleViolationWindow:getChildById('ipBanCheckBox')
                             :isChecked()
    ...
    elseif comment == "" then
        ...
        g_game.reportRuleViolation(target, reason, action, comment, statement,
                                   statementId, ipBanishment)
        hide()
    end
end
