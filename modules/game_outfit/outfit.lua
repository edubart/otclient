...
localPlayerEvent = EventController:new(LocalPlayer, {
	onOutfitChange = function(creature)
      ...
      outfit = creature:getOutfit()
			...
			for i, obj in pairs(outfits) do
				obj[3] = availableAddons
			end
		else
      availableAddons = outfits[currentOutfit][3]
		...
		for k, addon in pairs(addons) do
			addon.widget:setEnabled(availableAddons == 3 or addon.value == availableAddons)
...
controller:gameEvent('onOpenOutfitWindow', function(creatureOutfit, outfitList, creatureMount, mountList,
